...
{
    return (bool) ((base->CTRL >> SPI_CTRL_MODE_SHIFT) & 1U);
}
...
{
    return (bool) (((base->STS) >> (uint8_t) statusFlag) & 1U);
}
...
{
    return (bool) (((base->INTE) >> (uint8_t) interruptSrc) & 1U);
}
...
{
    base->CTRL = (base->CTRL & (~SPI_CTRL_TXDMAEN_MASK)) | ((uint32_t) enable << SPI_CTRL_TXDMAEN_SHIFT);
}
...
{
    (base->CTRL) = (base->CTRL & (~SPI_CTRL_RXDMAEN_MASK)) | ((uint32_t) enable << SPI_CTRL_RXDMAEN_SHIFT);
}
#endif /* !defined(FEATURE_SPI_LITE_VERSION) */
...
{
    return (spi_prescaler_t) (((uint32_t) ((base->TXCFG) & SPI_TXCFG_PRESCALE_MASK)) >> SPI_TXCFG_PRESCALE_SHIFT);
}
...
static inline void SPI_SetSamplingPoint(SPI_Type *base, bool isSamplingPointDelayed)
{
    ...
    spi_tmp |= ((uint32_t) isSamplingPointDelayed << SPI_CTRL_SPDEN_SHIFT);
    base->CTRL = spi_tmp;
}
