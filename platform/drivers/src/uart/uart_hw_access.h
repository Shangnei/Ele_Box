...
{
    return (0x01 << ((base->TXFIFO & UART_TXFIFO_SIZE_MASK) >> UART_TXFIFO_SIZE_SHIFT));
}
...
{
    return (0x01 << ((base->RXFIFO & UART_RXFIFO_SIZE_MASK) >> UART_RXFIFO_SIZE_SHIFT));
}
...
static inline void UART_EnableTxCts(UART_Type * base, bool enable)
{
    ...
    base->CTRL0 |= UART_CTRL0_TXCTSE(enable);
}
 ...
 */
static inline void UART_SetTxCtsDetectionTime(UART_Type * base, uart_tx_cts_detection_time_t time)
{
    base->CTRL0 &= ~UART_CTRL0_TXCTSC_MASK;
    base->CTRL0 |= UART_CTRL0_TXCTSC(time);
}
...
static inline void UART_EnableRxRts(UART_Type * base, bool enable)
{
    ...
    base->CTRL0 |= UART_CTRL0_RXRTSE(enable);
}
...
{
    return ((base->CTRL0 & UART_CTRL0_RXRTSE_MASK) >> UART_CTRL0_RXRTSE_SHIFT);
}
...
static inline void UART_EnableTxRts(UART_Type * base, bool enable)
{
    ...
    base->CTRL0 |= UART_CTRL0_TXRTSE(enable);
}
...
{
    return ((base->CTRL0 & UART_CTRL0_TXRTSE_MASK) >> UART_CTRL0_TXRTSE_SHIFT);
}
...
#if FEATURE_UART_HAS_ONE_WIRE_MODE_SUPPORT
/*!
...
static inline void UART_EnableLoopMode(UART_Type * base, bool enable)
{
    ...
    base->CTRL1 |= UART_CTRL1_LOOP(enable);
}
...
static inline void UART_EnableOneWireMode(UART_Type * base, bool enable)
{
    ...
    base->CTRL1 |= UART_CTRL1_ONEWIRE(enable);
}
